/* ============================================================
   PR Explorer · app-claude-v20.js · Midnight Teal
   V2.0: Topo default, Filter-Fix, Solo-Modus, Linienstil,
...
const F = { region:'all', status:'all', distMin:0, distMax:999, driveKmMin:0, driveKmMax:999, driveMinMin:0, driveMinMax:9999, elevUpMin:0, elevUpMax:99999 };
let _fb = {};
...
/* FILTER */
function regionFiltered(){ return DATA.filter(r=>{ if(getSt(r.id)==='skip')return false; if(F.region!=='all'&&groupOf(r)!==F.region)return false; if(F.status!=='all'&&getSt(r.id)!==F.status)return false; const q=S.query.trim().toLowerCase(); if(q&&!(r.id+' '+r.name+' '+regionLabel(r)).toLowerCase().includes(q))return false; return true; }); }
...
function renderLayers(){
  ...
  list.forEach(r=>{
    ...
    if(cfg.layers.markers&&r.lat&&r.lon){const st=getSt(r.id),col=levelColor(r.level),nr=r.id.replace('PR ','');const fav=favs.has(r.id)?'<span class="pin-fav"></span>':'';const w=Math.round(42*ps),h=Math.round(24*ps),fs=Math.round(9.5*ps);const html=`<div class="pr-pin-inner"><div class="pin-tag" style="background:${col};font-size:${fs}px">${nr}${fav}<span class="pin-sd ${st}"></span></div><div class="pin-tail" style="border-top-color:${col}"></div></div>`;const ico=L.divIcon({html,className:'pr-pin',iconSize:[w,h],iconAnchor:[w/2,h]});const m=L.marker([r.lat,r.lon],{icon:ico,riseOnHover:true,keyboard:false});m._prId=r.id;m.on('click',()=>openDetail(r.id,true));m.addTo(lgMarkers);}
  });
  updateZoom();
}
function highlightPin(id){ lgMarkers.eachLayer(m=>{const el=m.getElement();if(el)el.classList.toggle('pin-sel',m._prId===id);}); }
function soloOnMap(id){ cfg.soloMode=true;cfg.soloId=id;cfg.layers.tracks=true;cfg.layers.drive=true;cfg.layers.markers=true;saveCfg();renderLayers();setTab('map');const r=DATA.find(x=>x.id===id);if(r){setTimeout(()=>{const b=routeBounds(r);if(b)map.flyToBounds(b,{paddingTopLeft:[20,120],paddingBottomRight:[20,100],maxZoom:14,duration:.9});toast(`${id} · Tippe Karte für alle PR`);},200);} }
...
function renderFilterSheet(){
  const rSet=regionFiltered(),b=computeFilterBounds(rSet);_fb=b;
  ['dist','driveKm','driveMin','elevUp'].forEach(k=>{const lo=k+'Min',hi=k+'Max';if(F[hi]<F[lo])F[hi]=b[hi];F[lo]=Math.max(b[lo],F[lo]);F[hi]=Math.min(b[hi],F[hi]);if(F[hi]<F[lo])F[hi]=b[hi];});
  ...
  const sl=qs('#rangeSliders');if(sl){sl.innerHTML=dualSliderHtml('dist','Track-Länge',b.distMin,b.distMax,F.distMin,F.distMax,'km')+dualSliderHtml('drivekm','Anfahrt',b.driveKmMin,b.driveKmMax,F.driveKmMin,F.driveKmMax,'km')+dualSliderHtml('drivemin','Anfahrtszeit',b.driveMinMin,b.driveMinMax,F.driveMinMin,F.driveMinMax,'min')+dualSliderHtml('elevup','Höhenmeter ↑',b.elevUpMin,b.elevUpMax,F.elevUpMin,F.elevUpMax,'m');['dist','drivekm','drivemin','elevup'].forEach(id=>{dualMove(id,'lo');dualMove(id,'hi');});}
}
...
function setRegion(k){ F.region=k;F.distMin=0;F.distMax=999;F.driveKmMin=0;F.driveKmMax=999;F.driveMinMin=0;F.driveMinMax=9999;F.elevUpMin=0;F.elevUpMax=99999;renderFilterSheet();renderLayers();renderPanel(); }
function setSF(k){ F.status=F.status===k?'all':k;renderFilterSheet();renderLayers();renderPanel(); }
function resetFilters(){ F.region='all';F.status='all';S.query='';F.distMin=0;F.distMax=999;F.driveKmMin=0;F.driveKmMax=999;F.driveMinMin=0;F.driveMinMax=9999;F.elevUpMin=0;F.elevUpMax=99999;renderFilterSheet();renderLayers();renderPanel(); }
...
/* ICS */
function exportICS(id){ const r=DATA.find(x=>x.id===id);if(!r)return;const dt=cfg.tripStart?cfg.tripStart.replace(/-/g,'')+'T080000':new Date().toISOString().replace(/[-:]/g,'').slice(0,15);const ics=`BEGIN:VCALENDAR\nVERSION:2.0\nPRODID:-//PR Explorer Claude V2.0//DE\nBEGIN:VEVENT\nUID:pr-${id.replace(' ','-')}-${Date.now()}@pr-explorer\nDTSTAMP:${dt}Z\nDTSTART:${dt}\nDTEND:${dt.slice(0,8)}T180000\nSUMMARY:${r.id} · ${r.name}\nDESCRIPTION:${r.distanceKm||'?'} km · ${r.duration||'?'} · ↑${r.elevUp||r.high||'?'}m\nLOCATION:Madeira\\, Portugal\nEND:VEVENT\nEND:VCALENDAR`;const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([ics],{type:'text/calendar'}));a.download=`${r.id.replace(' ','-')}.ics`;a.click();toast('Kalender-Event exportiert'); }
...
/* SWIPE GESTURES */
function addSwipeClose(el,closeFn,directions){
  if(!el||el._swipeInit)return;el._swipeInit=true;
  let sx=0,sy=0,st=0,dragging=false,axis=null;
  el.addEventListener('touchstart',e=>{sx=e.touches[0].clientX;sy=e.touches[0].clientY;st=Date.now();dragging=false;axis=null;},{passive:true});
  el.addEventListener('touchmove',e=>{const dx=e.touches[0].clientX-sx,dy=e.touches[0].clientY-sy;if(!dragging){const horiz=Math.abs(dx)>14&&Math.abs(dx)>Math.abs(dy)*1.4;const down=Math.abs(dy)>14&&dy>0&&Math.abs(dy)>Math.abs(dx)*1.4;if(horiz&&(directions.includes('left')||directions.includes('right'))){dragging=true;axis='h';}else if(down&&directions.includes('down')){dragging=true;axis='v';}}if(!dragging)return;e.preventDefault();if(axis==='h'){el.style.transform=`translateX(${dx}px)`;el.style.opacity=String(Math.max(0,1-Math.abs(dx)/180));}else{const c=Math.max(0,dy);el.style.transform=`translateY(${c}px)`;el.style.opacity=String(Math.max(0,1-c/200));}},{passive:false});
  el.addEventListener('touchend',e=>{if(!dragging){el.style.transform='';el.style.opacity='';return;}const dx=e.changedTouches[0].clientX-sx,dy=e.changedTouches[0].clientY-sy;const vel=axis==='h'?Math.abs(dx)/(Date.now()-st):Math.max(0,dy)/(Date.now()-st);const dismiss=axis==='h'?(Math.abs(dx)>90||vel>0.35):(dy>100||vel>0.35);if(dismiss){el.style.transition='transform .26s ease,opacity .22s ease';el.style.transform=axis==='h'?`translateX(${dx>0?'110%':'-110%'})`:'translateY(110%)';el.style.opacity='0';setTimeout(()=>{el.style.transition='';el.style.transform='';el.style.opacity='';closeFn();},260);}else{el.style.transition='transform .22s var(--sp),opacity .18s';el.style.transform='';el.style.opacity='';setTimeout(()=>el.style.transition='',240);}dragging=false;axis=null;},{passive:true});
}
function initAllSwipe(){ addSwipeClose(qs('#detailPanel'),closeDetail,['left','right','down']);addSwipeClose(qs('#panel'),()=>{if(S.tab!=='map')setTab('map');},['down']);addSwipeClose(qs('#filterSheet'),closeFilterSheet,['down']);addSwipeClose(qs('#settingsPanel'),closeSettings,['down']); }
