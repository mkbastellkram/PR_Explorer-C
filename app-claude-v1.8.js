/* ============================================================
   PR Explorer · app-claude-v1.8.js · Midnight Teal
   ============================================================ */
'use strict';
...
function renderLayers() {
  ...
  list.forEach(r => {
    if(cfg.layers.tracks && r.track?.length) {
      L.polyline(r.track,{
        color:cfg.gpxColor, weight:2.8, opacity:.82,
        lineCap:'round', lineJoin:'round', smoothFactor:1.2,
      }).addTo(lgTrack);
    }
    if(cfg.layers.drive && r.driveRoute?.length) {
      L.polyline(r.driveRoute,{
        color:cfg.kmlColor, weight:2.2, opacity:.65,
        dashArray:'8 5', lineCap:'round', lineJoin:'round', smoothFactor:1.4,
      }).addTo(lgDrive);
    }
...
function setRegion(k) { S.filters.region=k; renderFilterSheet(); renderLayers(); renderPanel(); }
function setSF(k)     { S.filters.status = S.filters.status===k?'all':k; renderFilterSheet(); renderLayers(); renderPanel(); }
function resetFilters(){ S.filters={region:'all',status:'all'}; S.query=''; renderFilterSheet(); renderLayers(); renderPanel(); }
  ...
  if(S.tab==='overview') {
    ...
    h=`<div class="search-row">
      <input class="search-input" placeholder="PR suchen…" value="${S.query}"
        oninput="S.query=this.value;renderLayers();renderPanel()">
    </div>
...
function renderDetail() {
  ...
  const links = [
    {label:'Madeira',bg:'#1a73e8',em:'🌐', url:r.officialUrl},
    {label:'Instagram',bg:'linear-gradient(45deg,#f09433,#dc2743,#bc1888)',em:'📸',url:`https://www.instagram.com/explore/tags/${encodeURIComponent(r.id.replace(' ','').toLowerCase())}/`},
    {label:'Maps',bg:'#4285f4',em:'🗺️',url:r.startUrl},
    {label:'Anfahrt',bg:'#34a853',em:'🧭',url:r.driveUrl},
    {label:'YouTube',bg:'#ff0000',em:'▶️',url:`https://www.youtube.com/results?search_query=madeira+${encodeURIComponent(r.id)}`},
    {label:'Komoot',bg:'#6bc46d',em:'🥾',url:`https://www.komoot.com/search?q=madeira+${encodeURIComponent(r.name)}`},
    {label:'Strava',bg:'#fc4c02',em:'🏃',url:`https://www.strava.com/segments/explore?bounds=32.6,-17.3,32.9,-16.6`},
    {label:'Suche',bg:'#4285f4',em:'🔍',url:`https://www.google.com/search?q=madeira+${encodeURIComponent(r.id)}+${encodeURIComponent(r.name)}`},
  ].map(l=>`<a class="lk" href="${l.url||'#'}" target="_blank" rel="noopener">
    <div class="li" style="background:${l.bg};border-radius:11px">${l.em}</div>
    <span>${l.label}</span>
  </a>`).join('');
...
function renderSettings() {
  ...
  qs('#settingsContent').innerHTML=`
    ...
    <div class="s-footer">PR Explorer · Claude V1.8 · Midnight Teal<br>Alle Einstellungen lokal gespeichert.</div>`;
}
...
function bind() {
  ...
  qs('#shareBtn').onclick=()=>toast('Teilen kommt in V1.9');
...
Object.assign(window,{
  ...
  resetFilters, setRegion, setSF, toggleRegions,
